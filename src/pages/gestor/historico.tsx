...
export default function Historico({ tasks }:any) {
  ...
  return (
    <div>
      <Head>
        <title>Historico de ordens de serviço - Gestor</title>
      </Head>
      <SidebarGestor />

      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold text-gray-900">Historico de ordens de serviço</h1>
        </div>
      </header>
      <main>
      ...
      >
        <Box sx={{
          ...
          {taskOpened ? <OrderService item={taskOpened} closeButton={(e:any) => {handleClose()}}/> : ''}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  let tasks = await apiClient.get('/tasks/all/completed').then(response => {
